...
<div id="cart" class="w3-container city" <?php if(!isset($_GET['tab'])){ ?> style="display:none" <?php } ?> >
    <h2>購物車</h2> 
	 ...
	 <?php 
		...
		<table style="width:900px">
			...
			<tr>
				<th width="50px">全選&nbsp&nbsp<input type="checkbox" name="all" onclick="check_all(this,'cart[]')"/></th>
				<th width="50px">
					<input type="submit" name="button" id="button" value="勾選刪除" class="btn" 
					onClick="this.form.action='cart_click_delete.php';this.form.submit();"/>
				</th>
			...
			?>
			 ...
			 <tr>
				...
				<th>
					...
					onClick="this.form.action='cart_delete.php?id=<?php echo $cart_pid ?>';this.form.submit();"/>
				</th>
				<td><a href="product.php?id=<?php echo $rs_p['cart_pid'];?>"><?php echo $rs_p['p_name'];?></td>
				<td><?php echo $rs_p['p_price'];?></td>
				<td>
				<?php
						echo"<Select name=\"number[$cart_pid]\">";
		 ...
		 <?php
			...
			<tr>
				<th>
					<input type="submit"value="前往結帳" class="btn"
					onClick="this.form.action='cart_order.php';this.form.submit();"/>
				</th>
				<th></th>
				<th colspan="2"></th>
				<th >總計</th>
  ...
  <div id="order" class="w3-container city" style="display:none">
    ...
    <?php 
        ...
        $sql_p ="SELECT * FROM orders, product  WHERE  order_mid = $mid AND p_id = order_pid ORDER BY order_id";
        $result_p = $conn->query($sql_p);?>
    ...
    <table style="width:900px">
        ...
        for($i=1;$i<=$result_p->num_rows;$i++){
		 ...
		 $rs_p = $result_p->fetch_assoc();
		 $order_pid = $rs_p['order_pid'];
		?>
		<form  method="post" action="cart_test.php">
		 <tr>
				<td><?php echo $rs_p['order_id'];?></td>
				<td><a href="product.php?id=<?php echo $rs_p['order_pid'];?>"><?php echo $rs_p['p_name'];?></td>
				<td><?php echo $rs_p['p_price'];?></td>
				<td><?php echo $rs_p['order_amount'];?></td>
				<td><?php echo $rs_p['p_price']*$rs_p['order_amount'];?></td>

		 </tr>
		 
     <?php
		$totalprice += $rs_p['p_price']*$rs_p['order_amount'];
	 } ?>
		<tr>
			
			<th colspan="3"></th>
			<th width="300px"> 總計</th>
